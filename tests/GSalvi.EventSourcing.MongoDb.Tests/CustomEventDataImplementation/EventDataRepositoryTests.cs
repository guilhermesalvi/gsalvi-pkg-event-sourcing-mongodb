...
namespace GSalvi.EventSourcing.MongoDb.Tests.CustomEventDataImplementation;

[Collection(nameof(TestsFixtureCollection))]
public class EventDataRepositoryTests
{
    private readonly EventSourcingDbContext<CustomEventData> _context;
    private readonly IEventDataRepository<CustomEventData> _repository;
    private readonly Fixture _fixture;

    public EventDataRepositoryTests(TestHost<CustomEventDataTestStartup> testHost)
    {
        _repository = testHost.ServiceProvider.GetRequiredService<IEventDataRepository<CustomEventData>>();
        _context = testHost.ServiceProvider.GetRequiredService<EventSourcingDbContext<CustomEventData>>();
    ...
    public async Task AddAsync_ShouldAddOneItemInDatabase()
    {
        ...
        result.UserId.Should().Be(eventData.UserId);
